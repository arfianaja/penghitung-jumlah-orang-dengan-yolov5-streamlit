...
def detect(opt, stframe, person1, person2, fps_rate, class_id):
    ...
    for frame_idx, (path, img, im0s, vid_cap, s) in enumerate(dataset):
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if det is not None and len(det):
                ...
                if len(outputs) > 0:
                    for j, (output, conf) in enumerate(zip(outputs, confs)):
                        ...
                        # count_obj(bboxes,w,h,id, names[c], data_person, data_bus, data_truck, data_motor)
            ...
            if show_vid:
                ...
                # Left Lane Line
                cv2.line(im0, (0, h-200), (1300, h-200), (255, 0, 0), thickness=3)

                # Right Lane Line
                cv2.line(im0,(1300,h-200),(0,h-200),(0, 0, 255),thickness=3)
                
                thickness = 3 # font thickness
                font = cv2.FONT_HERSHEY_SIMPLEX
                fontScale = 1.2 
                person1.markdown(f"<h3> {str(up_count)} </h3>", unsafe_allow_html=True)

                person2.markdown(f"<h3> {str(down_count)} </h3>", unsafe_allow_html=True)
...
def count_obj(box,w,h,id,direct,cls):
    global up_count,down_count,tracker1, tracker2, person_count, truck_count
...
# reset id in data
def reset():
    global up_count,down_count, already
    up_count = 0
    down_count = 0
    already = []
...
def parse_opt():
    ...
    # class 0 is person, 1 is bycicle, 2 is person... 79 is oven
